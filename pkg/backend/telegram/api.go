...
// getUpdates long-polls. Only plain messages are subscribed to: re-delivering
// an edited answer would submit a second turn to an agent that already acted on
// the first.
...
// waiting for.
func (a *botAPI) sendMessage(ctx context.Context, chatID int64, threadID int, text string, replyTo int) (int, error) {
	params := url.Values{
...
func (a *botAPI) editMessageText(ctx context.Context, chatID int64, messageID int, text string) error {
	return a.call(ctx, "editMessageText", url.Values{
		"chat_id":                  {strconv.FormatInt(chatID, 10)},
